...
- ISR 支持：中断上下文仅允许零超时的 `osSemaphoreAcquire`/`osMessageQueuePut/Get`，以及 `osSemaphoreRelease`、`osEventFlagsSet/Clear` 等释放型 API；创建/删除对象、`osTimer*`、`osMutex*`、`osEventFlagsWait` 均返回 `osErrorISR`。
